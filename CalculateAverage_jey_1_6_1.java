...
public class CalculateAverage_jey_1_6_1 {
    ...
    static class MNode {
        ...
        MNode(ByteArrWrapper key, ResultRow val) {
            this.key = key;
            this.val = val;
        }
    }
    static class JHmap {
        final int NO_OF_BUCKETS = 512;
        final int MAX_BUCKET_SIZE = 4;
        MNode[] buckets;

        JHmap() {
            long start = System.currentTimeMillis();
            this.buckets= new MNode[NO_OF_BUCKETS*MAX_BUCKET_SIZE];

        }

        ResultRow get(ByteArrWrapper key) {
            int offset = key.hashCode() & 0x1ff ;
            for(int i=offset; i<offset+MAX_BUCKET_SIZE; i++) {
        ...
        void put(ByteArrWrapper key, ResultRow val) {
            int offset = key.hashCode() & 0x1ff  ;
            for(int i=offset; i<offset+MAX_BUCKET_SIZE; i++) {
                if(buckets[i] == null) {
                    buckets[i] = new MNode(key, val);
                    return;
                }
    ...
    static class ByteArrWrapper {
        ...
        public ByteArrWrapper(byte[] arr, int l) {
            this.wrapped = arr;
            this.l = l;
            this.hashCode  = arr[0] << 24 | arr[1] << 16 | arr[l-2] << 8 | arr[l-1];
...
//        HashMap<ByteArrWrapper, ResultRow> hMap = new HashMap<>();
        ...
        void process(byte[] arr, int rL) {
            for (int i=0; i<rL; i++) {
                ...
                if (x == '\n') {
                    ...
                    ResultRow existing = jmap.get(key);
                    if (existing == null) {
                        existing = new ResultRow(temp);
                        jmap.put(new ByteArrWrapper(Arrays.copyOf(buf, key.l), key.l), existing);
                    }
                    else {
        ...
        void print() throws IOException {
            ...
            for(var node: this.jmap.buckets) {
                if(node == null) {
                    continue;
                }
                node.val.calculateMean();
                map.put(new String(node.key.wrapped, StandardCharsets.UTF_8), node.val);
            }
...
//            System.out.println(map);
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (AffinityLock al = AffinityLock.acquireLock(9)) {
            ...
            for (int i = 0; i < tries; i++) {
                ...
                List<Long> rt = new ArrayList<>(100);
