...
public class CalculateAverage_jey_1_10 {
    // java --add-exports java.base/jdk.internal.util=ALL-UNNAMED CalculateAverage_jey_1_5.java

    static final File FILE = new File("/home/j/development/1brc/measurements.txt");
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < tries; i++) {
            ...
            ChunkedProcessor[] processors = new ChunkedProcessor[parallelism];
            long cS = 0;
            long start = System.currentTimeMillis();
            for (int c = 0; c < parallelism; c++, cS += chunkSize) {
                processors[c] = new ChunkedProcessor(FILE.getAbsolutePath(), cS, chunkSize, c);
            }
            ...
            for (int p = 0; p < parallelism; p++) {
                int finalP = p;
                var th = new Thread(() -> processors[finalP].run());
                th.start();
                thread[p] = th;
            }
            ...
            Map<ByteArrWrapper, ResultRow> aggMap = new HashMap<>();
            for (int p = 0; p < parallelism; p++) {
                for (var e : processors[p].bp.hMap.entrySet()) {
                    ResultRow current = e.getValue();
                    ResultRow inMap = aggMap.get(e.getKey());
                    if (inMap == null) {
                        inMap = new ResultRow(Integer.MAX_VALUE, Integer.MIN_VALUE, 0, 0);
                        aggMap.put(e.getKey(), inMap);
                    }
                    if (inMap.min > current.min) {
                        inMap.min = current.min;
                    }
                    if (inMap.max < current.max) {
                        inMap.max = current.max;
                    }
                    inMap.sum += current.sum;
                    inMap.count += current.count;
                }
            }

            Map<String, ResultRow> map = new TreeMap<>();
            for (var e : aggMap.entrySet()) {
                var m = e.getValue();
                m.calculateMean();
                var c = e.getKey();
